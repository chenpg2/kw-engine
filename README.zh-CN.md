...
| *"这是不是个好研究问题?帮我磨一下。"* | 跑**问题打磨循环**(见下) |
| *"这是我的项目和一些论文——帮我推进它。"* | 跑**问题驱动循环**(见下) |
...
在搭技术路线之前,值得先问一句:*这到底是不是个好问题?* 说:

> *"这是不是个好研究问题?帮我磨一下:<你的粗糙想法>。"*

Claude 跑 `/kw-question` —— 用七条判据(stakes、够具体、有竞争假设、可证伪、2 周可行的 pilot、负结果也有价值、有据可溯)逐条压力测试你的想法,把模糊的话题/gap 改写成真正可检验的问题,再用"最强审稿人反对意见"狠狠刁难一遍,产出一张 **Question Card**。然后问你要不要交给下面的项目循环。

> 方法来源:从公开的研究方法论(Alon、Platt 的 strong inference、DARPA Heilmeier Catechism、Hamming、Alvesson & Sandberg)以及 [good-question](https://github.com/Rimagination/good-question) 项目中蒸馏、重新表述。

### 推进你自己的项目(重头戏)

这才是引擎真正的用途。你带着一个研究问题、或一个半成形的想法(最好已经在上面磨过)、加一堆论文(可能还不够),想把项目往前推。说一句:

> *"我想设计一个更好的 X。这是我的论文。帮我捋出来。"*

Claude 会跑 `/kw-explore`,一个问题驱动的循环:

1. **框定问题** —— 把你的想法 decompose 成它的*结构*轴(并挑战这个框定、找出真正的难点),然后与你确认。
2. **吸收你的论文**,变成原则。
3. **把你的问题映射到库上** —— 显示哪些子问题已有被验证的机制,哪些还是**空白**(这就是"论文不够"具体显形的地方)。
4. **补空白** —— 提议定向要读的论文,获取并吸收你选中的,再重新映射。重复直到覆盖。
5. **组装设计** —— 把命中的原则拼成一个候选方案,*每个决策都标注它依赖哪条原则*,还没支撑的标红。
6. **交接** —— 把一份活的设计文档 + 决策依据 + 下一步写进*你的项目*,让新 session 无缝接续。

你的设计存在你的项目里;可复用的原则进知识库。每一轮新阅读都让设计更锐利。
