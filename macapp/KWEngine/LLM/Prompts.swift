//  Prompts.swift
//  KWEngine — the reasoning-layer prompts, ported from the kw-engine agents
//  (kw-reader, kw-distiller, kw-synthesizer) and process templates
//  (extract-template@v1, distill-rubric@v1).

import Foundation

enum Prompts {

    // MARK: - L1: faithful extraction (kw-reader, extract-template@v1)

    static let l1System = """
    You are kw-reader, the Layer-1 faithful-extraction stage of a methodology evolution engine. \
    ...
    static func l1User(paperId: String, pdfText: String) -> String {
        """
        Paper id: \(paperId)

        Full text extracted from paper/\(paperId).pdf follows. Produce the Layer-1 JSON record.
    ...
    // MARK: - L2: abstraction (kw-distiller, distill-rubric@v1)

    static let l2System = """
    You are kw-distiller, the Layer-2 abstraction stage of a methodology evolution engine. \
    ...
    duplicate it — report it under "existing_updates" with the provenance to add and (optionally) a link proposal.
    ...
    static func l2User(paperId: String, l1Markdown: String, existingProjectionsJSON: String) -> String {
        """
        Paper id: \(paperId)

        EXISTING principles in the library (projections; use for dedup and links):
        \(existingProjectionsJSON)

        Layer-1 faithful reading of memory/papers/\(paperId).md:
    ...
    // MARK: - L3: synthesis (kw-synthesizer)

    static let l3System = """
    You are kw-synthesizer: cross-paper Layer-3 synthesis over ALL Layer-2 principles of a \
    ...
    static func l3User(today: String, principlesJSON: String) -> String {
        """
        Today's date: \(today)
    ...
    // MARK: - Ask: problem → structure → matched mechanism

    static let askExtractSystem = """
    You map a user's problem description onto its STRUCTURAL signature, for searching a principle \
    ...
    static let askComposeSystem = """
    You are the retrieval reader of a methodology evolution engine. The user described a problem; \
    ...
    static func askComposeUser(question: String, records: String) -> String {
        """
        User's problem:
        \(question)
