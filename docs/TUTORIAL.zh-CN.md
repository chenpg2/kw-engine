...
在一头扎进技术方案之前,值得先检查一下:*这到底是不是个好问题?*

> **你说:** 这是不是个好研究问题?我觉得阴道微生物组的 CST 社区分型已经过时了——分类分辨率太低,离散分型也丢失了连续谱的信息。帮我磨一下。

**会发生:** Claude 跑一个**问题打磨循环**:
1. 用七条判据逐条压力测试你的想法(重不重要?够不够具体?有没有竞争假设?能不能证伪?两周内能不能跑个 pilot?负结果能不能学到东西?有没有证据支撑?);
2. 把模糊想法改写成一个真正可检验的问题;
3. 扮演**最刁钻的审稿人**,用最强反对意见刁难你;
4. 产出一张 **Question Card**——一页纸,包含问题、为什么重要、竞争假设、证伪条件、最佳下一步。

然后 Claude 会问:*"问题已经磨好了。要用 /kw-explore 做成技术路线吗?还是先停在这张卡片?"*

- 说**要** → 继续下面第 5 步。
- 说**不要** → 卡片留着写开题报告或组会讨论。

> 如果你已经有一个清晰的、成形的研究问题,可以跳过这一步直接到第 5 步。
